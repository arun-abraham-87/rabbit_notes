...
const StockVesting = ({ notes }) => {
  ...
  const getNext3MonthsVestings = (schedule) => {
    ...
    return schedule.data
      .filter(row => {
        ...
        return vestDate > today && vestDate <= threeMonthsFromNow;
      })
      .sort((a, b) => {
  ...
  useEffect(() => {
    ...
    // Group data by grant date
    const groupedByGrantDate = allData.reduce((acc, row) => {
      const grantDate = row.grantDate || 'Unknown';
      if (!acc[grantDate]) {
        acc[grantDate] = [];
      }
      acc[grantDate].push(row);
      return acc;
    }, {});

    // Convert grouped data to array format
    const processedData = Object.entries(groupedByGrantDate).map(([grantDate, data]) => {
      // Calculate totals for this grant date
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">Next 3 Months Vesting Schedule</h2>
              <button
                onClick={() => setShowNext3Months(false)}
            ...
            <div className="space-y-4">
              {vestingData.flatMap(schedule => {
                const next3MonthsVestings = getNext3MonthsVestings(schedule);
                if (next3MonthsVestings.length === 0) return [];

                return next3MonthsVestings.map((row, index) => (
                  <div key={`${schedule.grantDate}-${index}`} className="flex justify-between items-center">
                    <div>
                      <span className="text-green-500">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-300">
          ...
          <tbody>
            {isCollapsed ? (
              // Collapsed view - grouped by grant date
              ...
              vestingData.flatMap(schedule => 
                schedule.data
                  ...
                  .map((row, rowIndex) => (
                    <tr key={`${schedule.grantDate}-${rowIndex}`} className="hover:bg-gray-50">
                      <td className="px-6 py-4 border-b">
            ...
            {!isCollapsed && (
              <tr className="bg-gray-100 font-semibold">
                ...
                <td className="px-6 py-4 border-b">
                  {overallTotals.grantQuantity.toLocaleString()}
                </td>
                <td className="px-6 py-4 border-b">
                  {overallTotals.vestQuantity.toLocaleString()}
                </td>
                <td className="px-6 py-4 border-b">
                  {overallTotals.unvestQuantity.toLocaleString()}
                </td>
              </tr>
            )}
          </tbody>
